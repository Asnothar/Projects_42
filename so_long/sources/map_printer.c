...
/*   Updated: 2025/03/19 18:47:45 by abeaufil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	put_texture(t_data *data, int x, int y, char c)
{
	int		s;
	void	*img;
	...
	if (!img)
		return (ft_printf("Error : no textures found\n"), EXIT_FAILURE);
...
void	draw_map(t_data *data)
{
	...
	while (data->map[i] != NULL)
	{
		...
		while (data->map[i][j] != '\0')
		{
			...
			if (put_texture(data, j, i, data->map[i][j]) == EXIT_FAILURE)
			{
				end_game(data, 0);
				break ;
			}
...
int	end_game(t_data *data, int finish)
{
	if (finish == 1)
