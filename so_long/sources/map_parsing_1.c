...
/*   Updated: 2025/03/19 14:32:10 by abeaufil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_extension(char *file)
{
	...
	len = strlen(file);
	if (len < 5 || strcmp(file + len - 4, ".ber") != 0)
	{
		ft_printf(ERROR_BER);
		return (EXIT_FAILURE);
	}
...
int	check_lines(char **map)
{
	...
	while (map[i])
	{
		if (check_empty_line(map[i], 1))
			return (ft_printf(ERROR_EMPTY_LINE), EXIT_FAILURE);
		i++;
	}
	return (EXIT_SUCCESS);
}
...
int	is_map_enclosed(t_data *data)
{
	...
	while (i < data->width)
	{
		if (data->map[0][i] != '1' || data->map[data->height - 1][i] != '1')
			return (free_tab_char(data->map), EXIT_FAILURE);
		i++;
	}
	i = 0;
	while (i < data->height)
